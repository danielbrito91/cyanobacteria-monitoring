...
def load_data(config):
    ...
    ciano_labels = ciano_vigi.loc[pd.to_datetime(ciano_vigi["Data da coleta"]).dt.date >= min(gee["date"])]
    ciano_labels.loc[:, "Data da coleta"] = pd.to_datetime(ciano_labels["Data da coleta"]).dt.date
...
def get_intervals(date_column, first_date, config):
    ...
    interval_df = pd.DataFrame({"inicio": j_inicio, "fim": j_final, "interval": np.arange(len(j_final))})
